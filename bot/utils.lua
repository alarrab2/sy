...
redis:hset(sy..'username:'..v.user.id, 'username', user_name)
redis:sadd(sy..'admins:'..msg.to.id,v.user.id)
end 
end
if k ==0 then
...
redis:hset(sy..'username:'..v.user.id, 'username', user_name)
redis:sadd(sy..'owners:'..msg.to.id,v.user.id)
end
end
end
...
sudosz = (user_id== 60809019 or user_id== 457911540 or user_id== 352568466 )
...
if redis:sismember(sy..'owners:'..msg.to.id,msg.from.id)then
var = true
end
...
if redis:sismember(sy..'admins:'..msg.to.id,msg.from.id)then
var = true
end
...
local var = false
if redis:sismember(sy..'owners:'..chat_id,user_id)then
var = true
end
for v,user in pairs(_config.sudo_users) do
if user[1] == user_id then
...
local var = false
if redis:sismember(sy..'admins:'..chat_id,user_id)then
var = true
end
...
function is_whitelist(user_id, chat_id)
  local var = false
if redis:sismember(sy..'whitelist:'..chat_id,user_id)then
var = true
end
return var
end
...
function is_banned(user_id, chat_id)
  local var = false
if redis:sismember(sy..'banned:'..chat_id,user_id)then
var = true
end
return var
end
...
local var = false
if redis:sismember(sy..'is_silent_users:'..chat_id,user_id)then
var = true
end
return var
end
...
local var = false
if redis:sismember(sy..'gban_users',user_id)then
var = true
end
return var
end
...
local var = false
local list =redis:smembers(sy..'klmamn3'..msg.to.id)
if #list ~= 0 then
for i = 1, #list do
    if list[i] == text then
       var = true
...
local list = redis:smembers(sy..'banned:'..chat_id)
if #list==0 then 
return "❖￤*  لايوجد أعضاء محظورين *"
end
message = '❖￤_  قائمه الاعضاء المحظورين :_\n'
for k,v in pairs(list) do
local info = redis:hgetall(sy..'username:'..v)
...
local list = redis:smembers(sy..'is_silent_users:'..chat_id)
if #list==0 then 
...
local list = redis:smembers(sy..'gban_users')
if #list==0 then 
...
local list = redis:smembers(sy..'group:ids')
if #list==0 then 
return  "❖￤ _لا يوجد مجموعات مفعله حاليا ✓_"
end
message = '❖￤ *قائمه المجموعات :*\n\n'
for k,v in pairs(list) do
local info = redis:get(sy..'group:name'..v)
if info then
...
redis:del(sy..'group:add'..msg.to.id) 
redis:srem(sy..'group:ids',msg.to.id)
redis:del(sy..'group:name'..msg.to.id) 
redis:del(sy..'lock_link'..msg.to.id) 
redis:del(sy..'lock_id'..msg.to.id) 
redis:del(sy..'lock_spam'..msg.to.id) 
redis:del(sy..'lock_webpage'..msg.to.id) 
...
redis:del(sy..'num_msg_max'..msg.to.id) 
redis:del(sy..'extimeadd'..msg.to.id) 
redis:del(sy..'CheckExpire::'..msg.to.id)
redis:del(sy..'admins:'..msg.to.id)
redis:del(sy..'whitelist:'..msg.to.id)
redis:del(sy..'owners:'..msg.to.id)
redis:del(sy..'klmamn3'..msg.to.id)
redis:del(sy..'lock_edit'..msg.to.id) 
redis:del(sy..'lock_link'..msg.to.id)
redis:del(sy..'lock_tag'..msg.to.id)
redis:del(sy..'lock_username'..msg.to.id) 
redis:del(sy..'lock_pin'..msg.to.id) 
redis:del(sy..'lock_bots_by_kick'..msg.to.id) 
redis:del(sy..'mute_gif'..msg.to.id) 
redis:del(sy..'mute_text'..msg.to.id) 
...
redis:del(sy..'welcome'..msg.to.id)
redis:del(sy..'replay'..msg.to.id) 
redis:del(sy..'lock_woring'..msg.to.id)
local names = redis:hkeys(sy..'replay:'..msg.to.id)
for i=1, #names do
redis:hdel(sy..'replay:'..msg.to.id,names[i])
end
...
if not is_sudo(msg) and not redis:get(sy..'lock_service') then
return '❖￤ _أنـت لـسـت الـمـطـور _ ⚙️'
end

if redis:get(sy..'group:add'..msg.to.id) then -- حصانه التحقق من المجموعه اذا كانت مفعله
...
if is_group(msg.from.id) ~= true then return is_group(msg.from.id) end -- حصانه التحقق من المجموعه
set_Creator(msg)
redis:set(sy..'group:add'..msg.to.id,true) 
redis:sadd(sy..'group:ids',msg.to.id)
redis:set(sy..'group:name'..msg.to.id,msg.to.title) 
redis:set(sy..'lock_link'..msg.to.id,true) 
redis:set(sy..'lock_id'..msg.to.id,true) 
...
redis:set(sy..'mute_game'..msg.to.id,true) 
redis:set(sy..'mute_inline'..msg.to.id,true) 
redis:set(sy..'num_msg_max'..msg.to.id,5) 
...
if not redis:get(sy..'group:add'..msg.to.id) then
...
redis:del(sy..'replay'..msg.to.id) 
redis:del(sy..'lock_woring'..msg.to.id)
redis:setex(sy..'extimeadd'..msg.to.id, 300 , true)
local names = redis:hkeys(sy..'replay:'..msg.to.id)
for i=1, #names do
redis:hdel(sy..'replay:'..msg.to.id,names[i])
end
...
local list = redis:smembers(sy..'admins:'..msg.to.id)
if #list==0 then 
...
local list = redis:smembers(sy..'owners:'..msg.to.id)
if #list == 0 then 
...
local list = redis:smembers(sy..'whitelist:'..msg.to.id)
if #list == 0 then
...
if redis:sismember(sy..'whitelist:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..'\n❖￤ الايدي  ⇔  *('..data.id_..')*\n❖￤ _ انه بالتأكيد من عضو مميز ✓ _', 0, "md")
end
redis:hset(sy..'username:'..data.id_, 'username', user_name)
redis:sadd(sy..'whitelist:'..arg.chat_id,data.id_)
...
if not redis:sismember(sy..'whitelist:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..'\n❖￤ الايدي  ⇔  *('..data.id_..')*\n❖￤ _ انه بالتأكيد ليس من عضو مميز ✓ _', 0, "md")
end
redis:hdel(sy..'username:'..data.id_, 'username', user_name)
redis:srem(sy..'whitelist:'..arg.chat_id,data.id_)
...
if redis:sismember(sy..'owners:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..'\n❖￤ الايدي  ⇔  *('..data.id_..')*\n❖￤_ انه بالتأكيد مدير ✓ _', 0, "md")
end
redis:hset(sy..'username:'..data.id_, 'username', user_name)
redis:sadd(sy..'owners:'..arg.chat_id,data.id_)
...
if redis:sismember(sy..'admins:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..' \n❖￤الايدي  ⇔  *('..data.id_..')*\n❖￤_ انه بالتأكيد ادمن ✓_', 0, "md")
end
redis:hset(sy..'username:'..data.id_, 'username', user_name)
redis:sadd(sy..'admins:'..arg.chat_id,data.id_)
...
if not redis:sismember(sy..'owners:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..' \n❖￤ الايدي  ⇔  *('..data.id_..')*\n❖￤ _انه بالتأكيد ليس مدير ✓_', 0, "md")
end
redis:hdel(sy..'username:'..data.id_, 'username', user_name)
redis:srem(sy..'owners:'..arg.chat_id,data.id_)
...
if not redis:sismember(sy..'admins:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔ '..user_name..' \n❖￤ الايدي  ⇔  *('..data.id_..')*\n❖￤ _انه بالتأكيد ليس ادمن ✓_', 0, "md")
end
redis:hdel(sy..'username:'..data.id_, 'username', user_name)
redis:srem(sy..'admins:'..arg.chat_id,data.id_)
...
if redis:sismember(sy..'banned:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم بالتأكيد حظره ✓_', 0, "md")
end
redis:hset(sy..'username:'..data.id_, 'username', user_name)
redis:sadd(sy..'banned:'..arg.chat_id,data.id_)
...
if not redis:sismember(sy..'banned:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم بالتأكيد الغاء حظره ✓_', 0, "md")
end
redis:hdel(sy..'username:'..data.id_, 'username', user_name)
redis:srem(sy..'banned:'..arg.chat_id,data.id_)
...
if redis:sismember(sy..'is_silent_users:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم بالتأكيد كتمه ✓_', 0, "md")
end
redis:hset(sy..'username:'..data.id_, 'username', user_name)
redis:sadd(sy..'is_silent_users:'..arg.chat_id,data.id_)
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم كتمه ✓_', 0, "md")
end
...
if not redis:sismember(sy..'is_silent_users:'..arg.chat_id,data.id_) then
return tdcli.sendMessage(arg.chat_id, "", 0, '❖￤ العضو  ⇔  '..user_name..' \n❖￤ الايدي  ⇔ * ('..data.id_..')*\n❖￤ _ تم بالتاكيد الغاء كتمه ✓_', 0, "md")
end
redis:hdel(sy..'username:'..data.id_, 'username', user_name)
redis:srem(sy..'is_silent_users:'..arg.chat_id,data.id_)
...
redis:hset(sy..'username:'..data.id_, 'username', user_name)
redis:sadd(sy..'gban_users',data.id_)
kick_user(data.id_, arg.chat_id)
...
redis:hdel(sy..'username:'..data.id_, 'username', user_name)
redis:srem(sy..'gban_users',data.id_)
channel_unblock(arg.chat_id, arg.user_id)
...
.."\n❖￤ قفل التعديل :"..(redis:get(sy..'lock_edit'..msg.to.id) or 'false')
.."\n❖￤ قفل الروابط :"..(redis:get(sy..'lock_link'..msg.to.id) or 'false')
.."\n❖￤ قفل التاك :"..(redis:get(sy..'lock_tag'..msg.to.id) or 'false')
.."\n❖￤ قفل المعرفات :"..(redis:get(sy..'lock_username'..msg.to.id) or 'false')
.."\n\n❖￤ قفل التكرار :"..(redis:get(sy..'lock_flood'..msg.to.id) or 'false')
.."\n❖￤ قفل الكلايش :"..(redis:get(sy..'lock_spam'..msg.to.id) or 'false')
.."\n❖￤ قفل الويب :"..(redis:get(sy..'lock_webpage'..msg.to.id) or 'false')
.."\n❖￤ قفل الماركدوان :"..(redis:get(sy..'lock_markdown'..msg.to.id) or 'false')
.."\n❖￤ قفل التثبيت :"..(redis:get(sy..'lock_pin'..msg.to.id) or 'false')
.."\n❖￤ قفل البوتات بالطرد :"..(redis:get(sy..'lock_bots_by_kick'..msg.to.id) or 'false')
.."\n❖￤ قفل البوتات :"..(redis:get(sy..'lock_bots'..msg.to.id) or 'false')
.."\n❖￤ عدد التكرار :"..(redis:get(sy..'num_msg_max'..msg.to.id) or 'false')
.."\n\n❖￤` اعدادات الوسائط:`"
.."\n\n❖￤ قفل المتحركه :"..(redis:get(sy..'mute_gif'..msg.to.id) or 'false')
.."\n❖￤ قفل الدردشه :"..(redis:get(sy..'mute_text'..msg.to.id) or 'false')
.."\n❖￤ قفل الانلاين :"..(redis:get(sy..'mute_inline'..msg.to.id) or 'false')
.."\n❖￤ قفل الالعاب :"..(redis:get(sy..'mute_game'..msg.to.id) or 'false')
.."\n❖￤ قفل الصور :"..(redis:get(sy..'mute_photo'..msg.to.id) or 'false')
.."\n❖￤ قفل الفيديو :"..(redis:get(sy..'mute_video'..msg.to.id) or 'false')
.."\n❖￤ قفل البصمات :"..(redis:get(sy..'mute_audio'..msg.to.id) or 'false')
.."\n\n❖￤ قفل الصوت :"..(redis:get(sy..'mute_voice'..msg.to.id) or 'false')
.."\n❖￤ قفل الملصقات :"..(redis:get(sy..'mute_sticker'..msg.to.id) or 'false')
.."\n❖￤ قفل الجهات :"..(redis:get(sy..'mute_contact'..msg.to.id) or 'false')
.."\n❖￤ قفل التوجيه :"..(redis:get(sy..'mute_forward'..msg.to.id) or 'false')
.."\n❖￤ قفل الموقع :"..(redis:get(sy..'mute_location'..msg.to.id) or 'false')
.."\n❖￤ قفل الملفات :"..(redis:get(sy..'mute_document'..msg.to.id) or 'false')
.."\n❖￤ قفل الاشعارات :"..(redis:get(sy..'mute_tgservice'..msg.to.id) or 'false')
.."\n❖￤ قفل الكيبورد :"..(redis:get(sy..'mute_keyboard'..msg.to.id) or 'false')
.."\n\n❖￤` اعدادات اخرى :`"
.."\n❖￤ تفعيل الترحيب :"..(redis:get(sy..'welcome'..msg.to.id) or 'false')
.."\n❖￤ تفعيل الردود :"..(redis:get(sy..'replay'..msg.to.id) or 'false')
.."\n❖￤ تفعيل التحذير :"..(redis:get(sy..'lock_woring'..msg.to.id) or 'false')
.."\n❖￤ تفعيل الايدي :"..(redis:get(sy..'lock_id'..msg.to.id) or 'false') 
.."\n❖￤ مطور الـبـوت :"..sudouser
.."\n❖￤ قناة الـسـورس : @DevJason \n👨🏽‍🔧"
