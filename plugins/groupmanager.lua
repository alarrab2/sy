...
if  redis:get(sy..'group:add'..msg.to.id) then  -- حصانه اذا كانت المجموعه مفعله

if matches[1] == "ايدي" then
if not matches[2] and not msg.reply_id then
if redis:get(sy..'lock_id'..msg.to.id) then
...
rules = "*❖￤القوانين :*\n"..redis:get(sy..'rulse:msg'..msg.to.id)
end
return rules
end
...
redis:set(sy..'flood'..msg.to.id,matches[2]) 
...
redis:setex(sy..'name:witting'..msg.from.id,300,true)
...
redis:setex(sy..'photo:group'..msg.from.id,300,true)
return '📭¦ حسنآ عزيزي 🍁\n❖￤ الان قم بارسال الصوره  🛠'
end
end
if matches[1] == "ضع وصف" and is_mod(msg) then
redis:setex(sy..'about:witting'..msg.from.id,300,true) 
...
if matches[2] == "الترحيب" then
if redis:get(sy..'welcome:get'..msg.to.id) then
return "❖￤ _مرحبا عزيزي_\n❖￤ _تفعيل الترحيب مفعل مسبقاً_ ✓"
else
redis:set(sy..'welcome:get'..msg.to.id,true) 
return "❖￤ _مرحبا عزيزي_\n❖￤ _تم تفعيل الترحيب_ ✓"
end
end
...
redis:set(sy..'lock_woring'..msg.to.id,true)
return "❖￤ _مرحبا عزيزي_\n❖￤ _تم تفعيل التحذير_ ✓"
end end end
-------------------
...
if matches[2] == "الايدي" then
if not redis:get(sy..'lock_id'..msg.to.id) then
...
if matches[2] == "الترحيب" then
if not redis:get(sy..'welcome:get'..msg.to.id) then
...
if matches[2] == "التحذير" then
if not redis:get(sy..'lock_woring'..msg.to.id) then
...
redis:set(sy..'welcom:witting'..msg.from.id,true)
return "📭¦ حسنآ عزيزي  ✋🏿\n❖￤ ارسل كليشه الترحيب الان 🍃"
end
if matches[1] == "الترحيب"  and is_mod(msg) then
if redis:get(sy..'welcome:msg'..msg.to.id)  then
return redis:get(sy..'welcome:msg'..msg.to.id)
else
return "❖￤ مرحباً عزيزي\n❖￤ نورت المجموعه \n❖￤ تابع : @DevJason \n💂🏼‍♀️"
end
end
...
if not we_sudo(msg) and not redis:get(sy..'lock_brod') then 
return "❖￤ هذا الاوامر للمطور الاساسي فقط  🛠" 
end
redis:setex(sy..'fwd:'..msg.from.id,300, true)
...
redis:setex(sy..'fwd:all'..msg.from.id,300, true)
...
redis:setex(sy..'fwd:pv'..msg.from.id,300, true)
...
redis:setex(sy..'fwd:groups'..msg.from.id,300, true)
return "📭¦ _حسنآ الان ارسل الكليشه للاذاعه للمجموعات _🍃\n"	
end

----------------- استقبال الرسائل ---------------
if msg.text then
if (msg.text:match("^([https?://w]*.?telegram.me/joinchat/%S+)$") or msg.text:match("^([https?://w]*.?t.me/joinchat/%S+)$")) and redis:get(sy..'waiting_link'..msg.from.id) and is_owner(msg) then  -- استقبال الرابط
redis:set(sy..'linkgp'..msg.to.id,msg.text)
return "❖￤ _شكرأ لك 😻_\n❖￤ _تم حفظ الرابط بنجاح _✓"
end
----------------------------------------------------
if redis:get(sy..'welcom:witting'..msg.from.id) then -- استقبال كليشه الترحيب
redis:del(sy..'welcom:witting'..msg.from.id)
redis:set(sy..'welcome:msg'..msg.to.id,check_markdown(msg.text))
...
--------------------------------------------------------------------
if redis:get(sy..'rulse:witting'..msg.from.id) then --- استقبال القوانين
redis:del(sy..'rulse:witting'..msg.from.id)
redis:set(sy..'rulse:msg'..msg.to.id,check_markdown(msg.text))
return '❖￤ _مرحبآ عزيزي_\n❖￤ _تم حفظ القوانين بنجاح_✓\n❖￤ _اكتب [ القوانين ] لعرضها 📬_'
end
----------------------------------------------------------
if redis:get(sy..'name:witting'..msg.from.id) then --- استقبال الوصف
redis:del(sy..'name:witting'..msg.from.id)
tdcli.changeChatTitle(msg.to.id, msg.text, dl_cb, nil)
return "📭¦ تم تغير اسم المجموعه  ✋🏿\n❖￤ الان اسمه `"..check_markdown(msg.text).."` 🍃"
end
--------------------------------------------------------------
if redis:get(sy..'about:witting'..msg.from.id) then --- استقبال الوصف
redis:del(sy..'about:witting'..msg.from.id)
...
--------------------------------------------------------------------
if redis:get(sy..'fwd:all'..msg.from.id) then ---- استقبال رساله الاذاعه عام
redis:del(sy..'fwd:all'..msg.from.id)
...
tdcli.sendMessage(msg.to.id, 0, 0,'❖￤ تم اذاعه الى `'..redis:scard(sy..'users')..'` من المشتركين 👍🏿', 0)
end
---------------------------------------------------------------------------------
if redis:get(sy..'fwd:pv'..msg.from.id) then ---- استقبال رساله الاذاعه خاص
redis:del(sy..'fwd:pv'..msg.from.id)
local pv = redis:smembers(sy..'users')
for i = 1, #pv do
tdcli.sendMessage(pv[i], 0, 0, check_markdown(msg.text), 0)			
end
...
---------------------------------------------------------------------------------
if redis:get(sy..'fwd:groups'..msg.from.id) then ---- استقبال رساله الاذاعه خاص
redis:del(sy..'fwd:groups'..msg.from.id)
local groups = redis:smembers(sy..'group:ids')
for i = 1, #groups do
tdcli.sendMessage(groups[i], 0, 0, check_markdown(msg.text), 0)			
end
...
if redis:get(sy..'welcome:msg'..msg.to.id) then
welcome = redis:get(sy..'welcome:msg'..msg.to.id) 
else
welcome = "❖￤ مرحباً عزيزي\n❖￤ نورت المجموعه \n❖￤ تابع : @DevJason\n💂🏼‍♀️"
end
if redis:get(sy..'rulse:msg'..msg.to.id) then
rules = redis:get(sy..'rulse:msg'..msg.to.id)
else
rules = "❖￤ _مرحبأ عزيري_ 👋🏻 _القوانين كلاتي_ 👇🏻\n❖￤ _ممنوع نشر الروابط_ \n❖￤ _ممنوع التكلم او نشر صور اباحيه_ \n❖￤ _ممنوع  اعاده توجيه_ \n❖￤ _ممنوع التكلم بلطائفه_ \n❖￤ _الرجاء احترام المدراء والادمنيه _😅\n❖￤ _تابع _@DevJason 💤\n💂🏼‍♀️"
end
...
if redis:get(sy..'group:add'..msg.to.id) and redis:get(sy..'welcome:get'..msg.to.id) and msg.adduser ~= tonumber(our_id) then
if msg.adduser then
