...
if redis:get(sy..'bot:name') then
bot_name = redis:get(sy..'bot:name') 
else
bot_name = 'العراب'
end
...
if r=="المشتركين" and is_sudo(msg) and msg.to.type == 'pv'  then
  local users = '❖￤ عدد المشتركين  `'..redis:scard(sy..'users')..'` *مشترك في البوت *🍃'
return users
end

if r=="/start" and msg.to.type == 'pv'  then
 redis:sadd(sy..'users',msg.from.id)
...
redis:setex(sy..'namebot:witting'..msg.from.id,300,true)
...
if not we_sudo(msg) then return "❖￤️هذا الاوامر للمطور الاساسي فقط 🛠" end
redis:setex(sy..'addrd_all:'..msg.from.id,300 , true)
redis:del(sy..'allreplay:'..msg.from.id)
return "❖￤ _حسننا الان ارسل كلمة الرد العام _🍃\n"
end
------------------------------------------------------
if msg.text then
if redis:get(sy..'namebot:witting'..msg.from.id) then --- استقبال اسم البوت
redis:del(sy..'namebot:witting'..msg.from.id)
redis:set(sy..'bot:name',msg.text)
reload_plugins( )
return "❖￤ تم تغير اسم البوت  ✋🏿\n❖￤ الان اسمه `"..check_markdown(r).."` 🍃"
end

if redis:get(sy..'addrd_all:'..msg.from.id) then -- استقبال الرد لكل المجموعات
if not redis:get(sy..'allreplay:'..msg.from.id) then-- استقبال كلمه الرد لكل المجموعات
redis:setex(sy..'allreplay:'..msg.from.id, 300 , msg.text )
return "❖￤ _شكرأ لك 😻_\n❖￤ _الان ارسل جواب الرد _✔️" 
end
if redis:get(sy..'allreplay:'..msg.from.id) then -- استقبال جواب الرد لكل المجموعات
redis:hset(sy..'replay:all', redis:get(sy.."allreplay:"..msg.from.id), msg.text)
redis:del(sy..'addrd_all:'..msg.from.id)
return '('..check_markdown(redis:get(sy..'allreplay:'..msg.from.id))..')\n  ✓ تم اضافت الرد لكل المجموعات 🚀 '
end
end
-------------------------------------------------------------
if redis:get(sy..'addrd:'..msg.from.id) then -- استقبال الرد للمجموعه فقط
if not redis:get(sy..'replay1'..msg.from.id) then  -- كلمه الرد
redis:setex(sy..'replay1'..msg.from.id,300,msg.text)
return "❖￤ _شكرأ لك 😻_\n❖￤ _الان ارسل جواب الرد _✔️" 
end
if redis:get(sy..'replay1'..msg.from.id) then -- جواب الرد
redis:hset(sy..'replay:'..msg.to.id, redis:get(sy.."replay1"..msg.from.id), msg.text)
redis:del(sy..'addrd:'..msg.from.id)
return '('..check_markdown(redis:get(sy..'replay1'..msg.from.id))..')\n  ✓ تم اضافت الرد 🚀 '
end
end

--------------------------------------
if redis:get(sy..'delrd:'..msg.from.id) then
redis:del(sy..'delrd:'..msg.from.id)

if not redis:hget(sy..'replay:'..msg.to.id,msg.text) then
return '🗯هذا الرد ليش مضاف في قائمه الردود 🛠'
else
redis:hdel(sy..'replay:'..msg.to.id,msg.text)
return '('..check_markdown(msg.text)..')\n  ✓ تم مسح الرد 🚀 '
end
end
-------------------------------------
if redis:get(sy..'delrdall:'..msg.from.id) then
redis:del(sy..'delrdall:'..msg.from.id)
if not redis:hget(sy..'replay:all',msg.text) then
return '🗯هذا الرد ليش مضاف في قائمه الردود 🛠'
else
redis:hdel(sy..'replay:all',msg.text)
return '('..check_markdown(msg.text)..')\n  ✓ تم مسح الرد 🚀 '
end
end
...
if not is_owner(msg) then return"♨️ للمدراء فقط ! 🛠" end
local names = redis:hkeys(sy..'replay:all')
for i=1, #names do
redis:hdel(sy..'replay:all',names[i])
end
...
if not is_owner(msg) then return"♨️ للمدراء فقط ! 🛠" end
redis:set(sy..'delrdall:'..msg.from.id,true)
return "❖￤ حسننا عزيزي  ✋🏿\n❖￤ الان ارسل الرد لمسحها من  المجموعات 🍃"
end

if r== 'مسح رد' then
if not is_owner(msg) then return"♨️ للمدراء فقط ! 🛠" end
redis:set(sy..'delrd:'..msg.from.id,true)
...
if not redis:get(sy..'group:add'..msg.to.id) then return end

--------------------[Test Bot]----------------------------
if r=="اضف رد" then
if not is_owner(msg) then return"♨️ للمدراء فقط ! 🛠" end
redis:setex(sy..'addrd:'..msg.from.id,300 , true)
redis:del(sy.."replay1"..msg.from.id)
...
if string.match(r2, 'محمد')  or  string.match(r2, 'حموشي') or  string.match(r2, 'حمودي') or  string.match(r2, 'حموده')  or string.match(r2, 'حموش') then
return true
else
return false
end
...
----------------------------------------------
if redis:hget(sy..'replay:all', r) then
return check_markdown(redis:hget(sy..'replay:all', r))
elseif redis:hget(sy..'replay:'..msg.to.id, r) then
return check_markdown(redis:hget(sy..'replay:'..msg.to.id, r))
...
if string.len(r2) > 60 then return "❖￤ ما اكدر اقول اكثر من 60 حرف 🙌🏾" end
...
elseif r== "قله" and r2 then
if string.len(r2) > 60 then return "❖￤ ما اكدر اقله اكثر من 60 حرف 🙌🏾" end
if sudoname(r2) then return "📌 ما اكدر احجي عليه مستحيل 🕵🏻" end
if linkmsg(r2) then
return "انته لوتي عود ؟ هو اني نغل ههه يريدني انشر رابط او معرف 😪 " 
end
