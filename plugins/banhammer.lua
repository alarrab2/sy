...
if msg.to.type ~= 'pv' and redis:get(sy..'group:add'..msg.to.id)  then
...
--redis:hset(sy..'username:'..matches[2], 'username', user_name)
redis:sadd(sy..'banned:'..msg.to.id,matches[2])
kick_user(matches[2], msg.to.id)
return tdcli.sendMessage( msg.to.id, "", 0, '❖￤ الايدي  ⇔  *('..matches[2]..')*\n❖￤ _ تم حظره ✔️_', 0, "md")
end
if matches[2] and string.match(matches[2], '@[%a%d_]') then
...
--redis:hset(sy..'username:'..matches[2], 'username', user_name)
redis:srem(sy..'banned:'..msg.to.id,matches[2])
channel_unblock(msg.to.id, matches[2])
...
--redis:hset(sy..'username:'..matches[2], 'username', user_name)
redis:sadd(sy..'is_silent_users:'..msg.to.id,matches[2])
return tdcli.sendMessage(msg.to.id, "", 0, '❖￤ الايدي  ⇔  *('..matches[2]..')*\n❖￤ _ تم كتمه ✔️_', 0, "md")
end
if matches[2] and string.match(matches[2], '@[%a%d_]') then
...
--redis:hset(sy..'username:'..matches[2], 'username', user_name)
redis:srem(sy..'is_silent_users:'..msg.to.id,matches[2])
...
if #redis:smembers(sy..'banned:'..msg.to.id) ==0 then
return "*❖￤ لا يوجد مستخدمين محظورين  *"
end
redis:del(sy..'banned:'..msg.to.id)
----------------------------------------
...
if #redis:smembers(sy..'is_silent_users:'..msg.to.id) ==0 then
return "*❖￤ لا يوجد مستخدمين مكتومين في المجموعه *"
end
redis:del(sy..'is_silent_users:'..msg.to.id)
return "*❖￤ تم مسح قائمه الكتم*"
end
if matches[2] == 'المميزين'  then
if #redis:smembers(sy..'whitelist:'..msg.to.id) ==0 then
...
if #redis:smembers(sy..'gban_users') ==0 then
return "*❖￤ لا يوجد مستخدمين محظورين عام في المجموعه *"
end
redis:del(sy..'gban_users')
...
--redis:hset(sy..'username:'..matches[2], 'username', user_name)
redis:sadd(sy..'gban_users',matches[2])
kick_user(matches[2], msg.to.id)
...
--redis:hset(sy..'username:'..matches[2], 'username', user_name)
redis:srem(sy..'gban_users',matches[2])
channel_unblock(msg.to.id, matches[2])
